...
(() => {
  ...
  function renderTable() {
    ...
    // Update sort indicators, titles, selection button labels, and chart
...
function updateCategoryChart() {
  try {
    ...
    if (!state.catChart) {
            ...
            tooltip: {
              callbacks: {
                label: (ctx) => {
                  ...
                  try {
                    return `${ctx.label}: ${window.DC ? window.DC.state && typeof window.DC.state === 'object' ? '' : '' : ''}${humanizeBytes(bytes)}`;
                  } catch {
      ...
      state.catChart.data.datasets[0].backgroundColor = labels.map((_, i) => baseColors[i % baseColors.length]);
